...
// API Endpoints to monitor (hardcoded from config)
const API_ENDPOINTS = [
  {
    id: "CCTAPIEA",
...
async function monitorEndpoint(endpoint) {
  try {
    ...
    const healthCheck = await makeRequest(endpoint);
...
function startMonitoring() {
  ...
  log(`Monitoring ${API_ENDPOINTS.filter(e => e.enabled).length} endpoints`, 'INFO');
