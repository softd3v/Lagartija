          ...
          <tr>
            <td style="background-color: #f8fafc; padding: 24px; border-radius: 0 0 12px 12px; text-align: center;">
              <p style="margin: 0; color: #64748b; font-size: 13px;">
                This is an automated alert from Xentinel API Monitor
              </p>
...
// Generate database alert email template
const generateDatabaseAlertEmail = (
  endpoint: ApiEndpoint,
  healthCheck: ApiHealthCheck,
  alertType: 'down' | 'recovered'
): string => {
  const isDown = alertType === 'down';
  const statusColor = isDown ? '#ef4444' : '#10b981';
  const statusText = isDown ? 'DATABASE DISCONNECTED ⚠️' : 'DATABASE RECONNECTED ✅';
  const statusBg = isDown ? '#fee2e2' : '#d1fae5';
          ...
          <tr>
            <td style="padding: 32px;">
              ...
              <table width="100%" cellpadding="12" cellspacing="0" style="border: 1px solid #e2e8f0; border-radius: 8px; margin-bottom: 24px;">
                ...
                <tr style="background-color: #f8fafc;">
                  <td style="font-weight: 600; color: #475569; border-bottom: 1px solid #e2e8f0;">API Status</td>
                  <td style="color: #1e293b; border-bottom: 1px solid #e2e8f0;">${healthCheck.status === 'up' ? '✅ UP' : '❌ DOWN'}</td>
                </tr>
                <tr>
                  <td style="font-weight: 600; color: #475569; background-color: #f8fafc; border-bottom: 1px solid #e2e8f0;">Database Status</td>
                  <td style="color: #1e293b; border-bottom: 1px solid #e2e8f0;">${healthCheck.databaseConnected ? '✅ Connected' : '❌ Disconnected'}</td>
                </tr>
                <tr style="background-color: #f8fafc;">
                  <td style="font-weight: 600; color: #475569; border-bottom: 1px solid #e2e8f0;">Response Time</td>
                  <td style="color: #1e293b; border-bottom: 1px solid #e2e8f0;">${healthCheck.responseTime}ms</td>
                </tr>
                <tr>
                  <td style="font-weight: 600; color: #475569; background-color: #f8fafc;">Status Code</td>
                  <td style="color: #1e293b;">${healthCheck.statusCode || 'N/A'}</td>
                </tr>
              </table>

              ${healthCheck.databaseError ? `
                <div style="background-color: #fef2f2; border: 1px solid #fecaca; padding: 16px; border-radius: 8px; margin-bottom: 24px;">
                  <h3 style="margin: 0 0 8px 0; color: #dc2626; font-size: 16px; font-weight: 600;">Database Error</h3>
                  <p style="margin: 0; color: #991b1b; font-size: 14px; font-family: monospace;">${healthCheck.databaseError}</p>
                </div>
              ` : ''}

              ${healthCheck.responseData ? `
                <div style="margin-bottom: 24px;">
                  <h3 style="margin: 0 0 12px 0; color: #1e293b; font-size: 18px; font-weight: 600;">Full Response Data</h3>
                  ${formatJSON(healthCheck.responseData)}
                </div>
              ` : ''}
            </td>
...
export async function sendTestEmail(): Promise<void> {
  ...
  await transporter.sendMail({
    from: `"${process.env.ALERT_FROM_NAME}" <${process.env.ALERT_FROM_EMAIL}>`,
    to: recipients.join(', '),
    subject: '✅ Xentinel API Monitor - Test Email',
    html: `
<!DOCTYPE html>
<html>
<body style="font-family: Arial, sans-serif; background-color: #f8fafc; padding: 40px;">
  <div style="max-width: 600px; margin: 0 auto; background: white; padding: 32px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    <h1 style="color: #3b82f6; margin: 0 0 16px 0;">🛡️ Xentinel API Monitor</h1>
